...
import { requireLvglStore } from "mcp-bridge/project-access";
...
function requireRuntime(store: any): any {
    const rt: any = store.runtime; // index.ts:174
    if (!rt) {
        throw new BridgeError(
            "NOT_FOUND",
            "Simulator is not running. Call run_simulator first."
        );
    }
    return rt;
}

export const simulatorHandlers: Record<string, Handler> = {
    // --- run_simulator: start Run mode (F5) ---------------------------------
    async run_simulator(params: any): Promise<any> {
        ...
        await waitFor(
            () => !!store.runtime && !store.runtime.error,
            RUN_TIMEOUT_MS
        );

        const rt: any = store.runtime;
        if (!rt) {
            throw new BridgeError(
                "INTERNAL",
                "Simulator failed to start (build error or runtime aborted)."
            );
        }
        if (rt.error) {
            throw new BridgeError(
                "INTERNAL",
                `Simulator reported an error: ${rt.error}`
            );
        }
        return {
            running: true,
    ...
    async stop_simulator(): Promise<any> {
        const store = requireLvglStore();
        if (!store.runtime) {
    ...
    async screenshot_simulator(): Promise<any> {
        const store = requireLvglStore();
        ...
        const liveCanvas: HTMLCanvasElement | undefined = rt.ctx?.canvas;
        ...
        const w: number = rt.displayWidth; // wasm-runtime.tsx:130
        const h: number = rt.displayHeight; // wasm-runtime.tsx:131
        if (!rt.lastScreen || !w || !h) {
            throw new BridgeError(
                "INTERNAL",
                "No simulator frame available yet; wait for the first frame."
            );
        }
        ...
        if (!ctx) {
            throw new BridgeError(
                "INTERNAL",
                "Could not create an offscreen canvas for the screenshot."
            );
        }
    ...
    pause_simulator(): any {
        ...
        if (!store.projectTypeTraits.hasFlowSupport) {
            throw new BridgeError(
                "UNSUPPORTED",
                "Pause requires a flow project with an active debugger; this is a no-flow LVGL project (nothing to pause)."
            );
        }
    ...
    resume_simulator(): any {
        const store = requireLvglStore();
        const rt = requireRuntime(store);
        if (!store.projectTypeTraits.hasFlowSupport) {
            throw new BridgeError(
                "UNSUPPORTED",
                "Resume requires a flow project with an active debugger; this is a no-flow LVGL project."
            );
        }
    ...
    async step_simulator(params: any): Promise<any> {
        const store = requireLvglStore();
        const rt = requireRuntime(store);
        if (!store.projectTypeTraits.hasFlowSupport) {
            throw new BridgeError(
                "UNSUPPORTED",
                "Single-step requires a flow project with an active debugger; this is a no-flow LVGL project (no flow queue to step)."
            );
        }
        if (!rt.isDebuggerActive || !rt.isPaused) {
            throw new BridgeError(
                "UNSUPPORTED",
                "Step is only valid while paused in the debugger; pause the simulator first."
            );
        }
        const kind: string =
            (params && params.kind) ? params.kind : "step-over";
        if (
            kind !== "step-into" &&
            kind !== "step-over" &&
            kind !== "step-out"
            ...
            throw new BridgeError(
                "BAD_PARAMS",
                `kind must be step-into|step-over|step-out (got "${kind}").`
            );
        }
