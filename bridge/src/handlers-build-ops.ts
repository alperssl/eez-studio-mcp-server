...
const EXT_BEGIN =
    "/* ${eez-mcp:ext_click_area} managed by set_ext_click_area — do not edit */";
...
function parseExtClickAreas(text: string): Map<string, number> {
 ...
 */
function writeExtClickAreas(text: string, map: Map<string, number>): string {
...
export const buildOpsHandlers: Record<string, Handler> = {
    ...
    set_ext_click_area(params: any) {
        const store = requireProjectStore();

        // Resolve the widget's C identifier (codegen names it objects.<identifier>).
        let identifier: string | undefined = params.identifier;
        if (!identifier && params.objID) {
            const obj: any = resolveObject(store, params.objID);
            identifier = obj && obj.identifier;
            if (!identifier) {
                throw new BridgeError(
                    "BAD_PARAMS",
                    "That widget has no identifier — set one with set_identifier first " +
        ...
        if (
            ...
            throw new BridgeError(
                "BAD_PARAMS",
                "identifier (a widget's C identifier) or objID of a widget with an identifier is required."
            );
        }
        ...
        if (!Number.isInteger(size) || size < 0) {
            throw new BridgeError(
                "BAD_PARAMS",
                "size (a non-negative integer of extra px on all sides; 0 removes) is required."
            );
        }

        const files = buildFiles(store);
        const fileName = params.fileName || "ui.c";
        const uic = files.find((f: any) => f.fileName === fileName);
        if (!uic) {
            throw new BridgeError(
                "NOT_FOUND",
                `No '${fileName}' build file. Available: ${files
                    .map((f: any) => f.fileName)
                    .join(", ")}.`
            );
        }
        const before: string = uic.template != undefined ? uic.template : "";
        if (!/void\s+ui_tick\s*\(\s*\)\s*\{/.test(before)) {
            throw new BridgeError(
                "UNSUPPORTED",
                `'${fileName}' has no ui_tick() to hook — cannot inject the ext_click_area applier.`
            );
        }
