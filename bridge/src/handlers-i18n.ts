...
function requireTexts(store: ProjectStoreLike): any {
    const texts = store.project.texts;
    if (!texts) {
        throw new BridgeError(
            "UNSUPPORTED",
            "Texts feature is not enabled; call enable_feature{key:'texts'} first."
        );
    }
    return texts;
}
...
interface ImportTable {
    languages: string[];
    rows: Array<{ resourceID: string; cells: Record<string, string> }>;
}

/** Parse import payload (csv/json/xliff) into a normalized {languages, rows} table. */
async function parseImport(format: string, data: any): Promise<ImportTable> {
    const fmt = String(format || "").toLowerCase();

    if (fmt === "json") {
        const obj = typeof data === "string" ? JSON.parse(data) : data;
        const languages: string[] = Array.isArray(obj.languages)
            ? obj.languages.map((l: any) => String(l))
            : [];
        const rawRows: any[] = Array.isArray(obj.resources)
            ? obj.resources
    ...
    if (fmt === "xliff") {
        ...
        const target: string = js.targetLanguage || js.trgLang || "";
        const source: string = js.sourceLanguage || js.srcLang || "";
        const languages: string[] = [];
        if (source) languages.push(source);
        if (target && target !== source) languages.push(target);
        const rows: Array<{ resourceID: string; cells: Record<string, string> }> = [];
    ...
    throw new BridgeError(
        "BAD_PARAMS",
        `Unsupported import format "${format}". Use csv, json or xliff.`
    );
}
...
export const i18nHandlers: Record<string, Handler> = {
    ...
    list_languages() {
        ...
        return {
            languages: texts.languages.map((l: any) => ({
    ...
    add_language(params: any) {
        const store = requireProjectStore();
        const texts = requireTexts(store);
        if (!params.languageID) {
            throw new BridgeError("BAD_PARAMS", "languageID is required.");
        }
        if (
            texts.languages.some(
                (l: any) => l.languageID === params.languageID
            )
        ) {
            throw new BridgeError(
                "BAD_PARAMS",
                `A language "${params.languageID}" already exists.`
            );
        }
        const language: any = createObject(
            store,
            { languageID: params.languageID } as any,
            Language
        );
    ...
    list_text_resources(params: any) {
        const store = requireProjectStore();
        const texts = requireTexts(store);
        const totalLangs = texts.languages.length;
        return {
            resources: texts.resources.map((r: any) => {
                const cells = (r.translations || []).filter(
                    (t: any) =>
                        !params.languageID ||
    ...
    add_text_resource(params: any) {
        const store = requireProjectStore();
        const texts = requireTexts(store);
        if (!params.resourceID) {
            throw new BridgeError("BAD_PARAMS", "resourceID is required.");
        }
        if (
            texts.resources.some(
                (r: any) => r.resourceID === params.resourceID
            )
        ) {
            throw new BridgeError(
                "BAD_PARAMS",
                `A text resource "${params.resourceID}" already exists.`
            );
        }
        const translations = texts.languages.map((l: any) => ({
            languageID: l.languageID,
            text: ""
        }));
        const resource: any = createObject(
            store,
            { resourceID: params.resourceID, translations } as any,
            TextResource
        );
    ...
    set_translation(params: any) {
        const store = requireProjectStore();
        const texts = requireTexts(store);
        if (params.text === undefined) {
            throw new BridgeError("BAD_PARAMS", "text is required.");
        }
        const resource = texts.resources.find(
            (r: any) => r.resourceID === params.resourceID
        );
        if (!resource) {
            throw new BridgeError(
                "NOT_FOUND",
                `No text resource "${params.resourceID}".`
            );
        }
        const cell = (resource.translations || []).find(
            (t: any) => t.languageID === params.languageID
        );
        if (!cell) {
            throw new BridgeError(
                "NOT_FOUND",
                `Language "${params.languageID}" has no cell in resource "${params.resourceID}". Add the language first.`
            );
        }
    ...
    set_widget_text_resource(params: any) {
        const store = requireProjectStore();
        const texts = requireTexts(store);
        ...
        if (widget.textType === undefined || widget.text === undefined) {
            throw new BridgeError(
                "UNSUPPORTED",
                `Widget ${widget.type} has no text property to bind.`
            );
        }
        const resource = texts.resources.find(
            (r: any) => r.resourceID === params.resourceID
    ...
    delete_language(params: any) {
        const store = requireProjectStore();
        const texts = requireTexts(store);
        const language = texts.languages.find(
            (l: any) => l.languageID === params.languageID
        );
        if (!language) {
            throw new BridgeError(
                "NOT_FOUND",
                `No language "${params.languageID}".`
            );
        }
    ...
    rename_language(params: any) {
        const store = requireProjectStore();
        const texts = requireTexts(store);
        if (!params.newLanguageID) {
            throw new BridgeError("BAD_PARAMS", "newLanguageID is required.");
        }
        const language = texts.languages.find(
            (l: any) => l.languageID === params.languageID
        );
        ...
        if (
            texts.languages.some(
                (l: any) => l.languageID === params.newLanguageID
            )
        ) {
            throw new BridgeError(
                "BAD_PARAMS",
                `A language "${params.newLanguageID}" already exists.`
            );
        }
    ...
    rename_text_resource(params: any) {
        const store = requireProjectStore();
        const texts = requireTexts(store);
        if (!params.newResourceID) {
            throw new BridgeError("BAD_PARAMS", "newResourceID is required.");
        }
        const resource = texts.resources.find(
            (r: any) => r.resourceID === params.resourceID
        );
        ...
        if (
            texts.resources.some(
                (r: any) => r.resourceID === params.newResourceID
            )
        ) {
            throw new BridgeError(
                "BAD_PARAMS",
                `A text resource "${params.newResourceID}" already exists.`
            );
        }
    ...
    delete_text_resource(params: any) {
        const store = requireProjectStore();
        const texts = requireTexts(store);
        const resource = texts.resources.find(
            (r: any) => r.resourceID === params.resourceID
    ...
    async import_texts(params: any) {
        const store = requireProjectStore();
        const texts = requireTexts(store);
        if (params.data === undefined || params.data === null) {
            throw new BridgeError("BAD_PARAMS", "data is required.");
        }
        ...
        try {
            ...
            for (const lang of table.languages) {
                if (!lang) continue;
                if (
                    !texts.languages.some((l: any) => l.languageID === lang)
                ) {
                    store.addObject(
                        texts.languages,
                        createObject(
                            store,
                            { languageID: lang } as any,
                            Language
                        )
                    );
                    languagesAdded++;
            ...
            for (const row of table.rows) {
                let resource = texts.resources.find(
                    (r: any) => r.resourceID === row.resourceID
                );
                if (!resource) {
                    const translations = texts.languages.map((l: any) => ({
                        languageID: l.languageID,
                    ...
                    resource = createObject(
                        store,
                        { resourceID: row.resourceID, translations } as any,
                        TextResource
                    );
                    store.addObject(texts.resources, resource);
                    resourcesAdded++;
    ...
    async export_texts(params: any) {
        const store = requireProjectStore();
        const texts = requireTexts(store);
        const fmt = String(params.format || "").toLowerCase();
        const langs: string[] = texts.languages.map((l: any) => l.languageID);

        const rows = texts.resources.map((r: any) => {
            const cells: Record<string, string> = {};
        ...
        if (fmt === "csv") {
            ...
            const body = rows.map((row: any) =>
                [
                    ...
                    ...langs.map((l: string) => csvEscape(row.cells[l] ?? ""))
                ].join(",")
            );
        ...
        if (fmt === "xliff") {
            ...
            const data: string = await xliff.js2xliff(js);
            return { data };
        }

        throw new BridgeError(
            "BAD_PARAMS",
            `Unsupported export format "${params.format}". Use csv, json or xliff.`
        );
    }
};
