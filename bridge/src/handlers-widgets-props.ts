...
import { LVGL_REACTIVE_FLAGS } from "project-editor/lvgl/lvgl-constants";
...
function assertOneOf(value: any, allowed: string[], label: string): void {
...
function assertStyleProp(name: string): void {
    if (!lvglPropertiesMap.get(name)) {
        throw new BridgeError("BAD_PARAMS", `Unknown style property "${name}".`);
    }
}
 ...
 */
function openLocalStyleCell(widget: any): { ls: any; def: any; cell: any } {
...
export const widgetsPropsHandlers: Record<string, Handler> = {
    // --- set_flag ------------------------------------------------------------
    ...
    set_flag(params: any) {
        ...
        if (!flag || typeof flag !== "string") {
            throw new BridgeError("BAD_PARAMS", "flag (string) is required.");
        }
        if (typeof params.on !== "boolean") {
            throw new BridgeError("BAD_PARAMS", "on (boolean) is required.");
        }
        if (LVGL_REACTIVE_FLAGS.indexOf(flag as any) !== -1) {
            throw new BridgeError(
                "BAD_PARAMS",
                `Flag "${flag}" is reactive and is not stored in widgetFlags; set it via update_widget {hiddenFlag|clickableFlag}.`
            );
        }
        const codes: any = getLvglFlagCodes(widget);
        if (Object.keys(codes).indexOf(flag) === -1) {
            throw new BridgeError(
                "BAD_PARAMS",
                `Flag "${flag}" is not available for this widget/LVGL version.`
            );
        }
        const widgetFlags = togglePipeToken(
            store,
            widget,
            "widgetFlags",
            flag,
    ...
    set_layout(params: any) {
        ...
        if (params.layout === "FLEX") {
            ...
            if (params.flexMainPlace !== undefined) {
                assertOneOf(
                    params.flexMainPlace,
                    VALID_FLEX_MAIN_PLACE,
                    "flexMainPlace"
                );
            ...
            if (params.flexCrossPlace !== undefined) {
                assertOneOf(
                    params.flexCrossPlace,
                    VALID_FLEX_CROSS_PLACE,
                    "flexCrossPlace"
                );
            ...
            if (params.flexTrackPlace !== undefined) {
                assertOneOf(
                    params.flexTrackPlace,
                    VALID_FLEX_TRACK_PLACE,
                    "flexTrackPlace"
                );
    ...
    set_scroll(params: any) {
        const store = requireLvglStore();
        const widget = resolveObject(store, params.objID);
        const props: any = {};
        if (params.scrollbarMode !== undefined) {
            assertOneOf(
                params.scrollbarMode,
                VALID_SCROLLBAR_MODE,
                "scrollbarMode"
            );
        ...
        if (Object.keys(props).length === 0) {
            throw new BridgeError(
                "BAD_PARAMS",
                "At least one of scrollbarMode/scrollDirection/scrollSnapX/scrollSnapY is required."
            );
        }
    ...
    set_grid_cell(params: any) {
        ...
        if (!wrote) {
            throw new BridgeError(
                "BAD_PARAMS",
                "At least one grid-cell property (colPos/colSpan/xAlign/rowPos/rowSpan/yAlign) is required."
            );
        }
