...
exports.flowReactiveHandlers = void 0;
...
function normalizeType(type) {
    ...
    if (t !== "literal" && t !== "expression") {
        throw new protocol_1.BridgeError("BAD_PARAMS", `type must be one of ${LEGAL_TYPES.join(", ")}, got "${t}".`);
    }
    return t;
}
function assertExpressionValue(type, value) {
    if (type === "expression" && typeof value !== "string") {
        throw new protocol_1.BridgeError("BAD_PARAMS", "expression value must be a string (an EEZ expression).");
    }
}
function safe(fn) {
    try {
...
exports.flowReactiveHandlers = {
    ...
    set_reactive_flag(params) {
        ...
        if (!prop) {
            throw new protocol_1.BridgeError("BAD_PARAMS", `flag must be HIDDEN or CLICKABLE, got "${params.flag}".`);
        }
    ...
    set_reactive_state(params) {
        const store = (0, project_access_1.requireLvglStore)();
        const widget = (0, project_access_1.resolveObject)(store, params.objID);
        const prop = STATE_PROP[params.state];
        if (!prop) {
            throw new protocol_1.BridgeError("BAD_PARAMS", `state must be CHECKED or DISABLED, got "${params.state}".`);
        }
        const type = normalizeType(params.type);
        assertExpressionValue(type, params.value);
        store.updateObject(widget, {
    ...
    render_flow(params) {
        ...
        const flow = (0, project_access_1.resolveFlow)(store, {
            page: params.page,
        ...
        const components = (flow.components || []).map((c) => ({
            ...
            width: c.width,
            height: c.height,
            label: safe(() => (0, store_1.getLabel)(c))
        }));
        const connectionLines = (flow.connectionLines || []).map((l) => ({
            source: l.source,
            output: l.output,
            target: l.target,
            input: l.input
        ...
        return {
            components,
            connectionLines,
            note: "flow canvas has no rasterizer; returning the wire-graph as data"
        };
    },
    ...
    async find_component(params) {
        const store = (0, project_access_1.requireProjectStore)();
        const raw = String(params.componentPath ?? "");
        const m = raw.match(/^(\d+)\.(\d+)$/);
        if (!m) {
            throw new protocol_1.BridgeError("BAD_PARAMS", `componentPath must be "flowIndex.componentIndex", got "${raw}".`);
        }
        ...
        const parts = await store.buildAssets();
        const map = parts?.GUI_ASSETS_DATA_MAP_JS;
        if (!map) {
            return { found: false, objID: null };
        }
        const flowMap = map.flows?.[flowIndex];
        const compMap = flowMap?.components?.[componentIndex];
        if (!compMap?.path) {
            return { found: false, objID: null };
        }
        const obj = (0, store_1.getObjectFromStringPath)(store.project, compMap.path);
        if (!obj || !obj.objID) {
            return { found: false, objID: null };
        }
