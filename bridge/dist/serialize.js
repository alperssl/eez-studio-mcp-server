"use strict";
// Serialize live LVGL widget objects into the plain WidgetNode/WidgetDetail wire shapes.
Object.defineProperty(exports, "__esModule", { value: true });
exports.widgetToNode = widgetToNode;
exports.widgetToDetail = widgetToDetail;
const object_1 = require("project-editor/core/object");
const project_access_1 = require("mcp-bridge/project-access");
// Properties handled explicitly or too heavy/circular to dump generically.
const SKIP_PROPS = new Set([
    "children",
    "localStyles",
    "eventHandlers",
    "objID",
    "type",
    "customInputs",
    "customOutputs",
    "asInputProperties",
    "asOutputProperties",
    "catchError"
]);
function readRect(widget) {
    return {
        left: widget.left ?? 0,
        top: widget.top ?? 0,
        width: widget.width ?? 0,
        height: widget.height ?? 0,
        leftUnit: widget.leftUnit ?? "px",
        topUnit: widget.topUnit ?? "px",
        widthUnit: widget.widthUnit ?? "px",
        heightUnit: widget.heightUnit ?? "px"
    };
}
function localStylesDefinition(widget) {
    const def = widget.localStyles?.definition;
    return def && typeof def === "object" ? def : {};
}
function widgetToNode(widget, deep = true) {
    const node = {
        objID: widget.objID,
        type: widget.type,
        identifier: widget.identifier || null,
        rect: readRect(widget),
        absoluteRect: (0, project_access_1.absoluteRect)(widget),
        useStyle: widget.useStyle || null,
        hasLocalStyles: Object.keys(localStylesDefinition(widget)).length > 0,
        children: []
    };
    if (typeof widget.text === "string") {
        node.text = widget.text;
    }
    const children = Array.isArray(widget.children)
        ? widget.children
        : [];
    if (deep) {
        node.children = children.map(c => widgetToNode(c, true));
    }
    else {
        node.children = children.map(c => widgetToNode(c, false));
    }
    return node;
}
function readProps(widget) {
    const out = {};
    const classInfo = (0, object_1.getClassInfo)(widget);
    const properties = classInfo?.properties || [];
    for (const p of properties) {
        const name = p.name;
        if (SKIP_PROPS.has(name)) {
            continue;
        }
        const v = widget[name];
        const t = typeof v;
        if (t === "string" || t === "number" || t === "boolean") {
            out[name] = v;
        }
    }
    return out;
}
function readEventHandlers(widget) {
    const handlers = Array.isArray(widget.eventHandlers)
        ? widget.eventHandlers
        : [];
    return handlers.map(eh => ({
        eventName: eh.eventName,
        handlerType: eh.handlerType,
        action: eh.action,
        userData: eh.userData ?? 0
    }));
}
function widgetToDetail(widget) {
    const node = widgetToNode(widget, false);
    return {
        ...node,
        props: readProps(widget),
        localStyles: localStylesDefinition(widget),
        eventHandlers: readEventHandlers(widget)
    };
}
