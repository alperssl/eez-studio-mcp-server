...
const project_1 = require("project-editor/project/project");
...
function requireFeature(key) {
    if (!key) {
        throw new protocol_1.BridgeError("BAD_PARAMS", "key is required.");
    }
 ...
 */
function isEffectivelyMandatory(store, feature) {
...
exports.settingsHandlers = {
    ...
    list_features() {
        ...
        return {
            features: (0, features_1.getProjectFeatures)().map((f) => ({
                key: f.key,
                displayName: f.displayName || f.name,
                enabled: !!(0, object_1.getProperty)(project, f.key),
    ...
    enable_feature(params) {
        ...
        if ((0, object_1.getProperty)(project, feature.key)) {
            return { key: feature.key, enabled: true };
        }
        const seed = feature.create();
        const newObj = (0, store_1.createObject)(store, seed, feature.typeClass, feature.key);
    ...
    disable_feature(params) {
        ...
        if (isEffectivelyMandatory(store, feature)) {
            throw new protocol_1.BridgeError("UNSUPPORTED", `Feature "${feature.key}" is mandatory for this project type and cannot be removed.`);
        }
        const values = { [feature.key]: undefined };
    ...
    add_project_import(params) {
        const store = (0, project_access_1.requireProjectStore)();
        const general = store.project.settings?.general;
        if (!general) {
            throw new protocol_1.BridgeError("NOT_FOUND", "Project has no general settings.");
        }
        if (!params.projectFilePath) {
            throw new protocol_1.BridgeError("BAD_PARAMS", "projectFilePath is required.");
        }
        const traits = store.project.projectTypeTraits;
        if (general.masterProject || traits.isApplet || traits.isIEXT) {
            throw new protocol_1.BridgeError("UNSUPPORTED", "Imports are not allowed for master-project / applet / IEXT projects.");
        }
        // importAs is unique across imports (matches ImportDirective.importAs unique:true).
        if (params.importAs) {
            if (projectImports(store).some((i) => i.importAs === params.importAs)) {
                throw new protocol_1.BridgeError("BAD_PARAMS", `importAs "${params.importAs}" is already used.`);
            }
        }
        // projectFilePath is a RelativeFile — persist it relative to the project.
        const relPath = store.getFilePathRelativeToProjectPath(params.projectFilePath);
        const seed = { projectFilePath: relPath };
        if (params.importAs) {
            seed.importAs = params.importAs;
        }
        const imp = (0, store_1.createObject)(store, seed, project_1.ImportDirective);
    ...
    remove_project_import(params) {
        const store = (0, project_access_1.requireProjectStore)();
        const imports = projectImports(store);
        let imp;
        if (params.importAs) {
            imp = imports.find((i) => i.importAs === params.importAs);
        }
        else if (params.projectFilePath) {
            const rel = store.getFilePathRelativeToProjectPath(params.projectFilePath);
            imp = imports.find((i) => i.projectFilePath === rel ||
                i.projectFilePath === params.projectFilePath);
        }
        else {
            throw new protocol_1.BridgeError("BAD_PARAMS", "Either importAs or projectFilePath is required.");
        }
        if (!imp) {
            throw new protocol_1.BridgeError("NOT_FOUND", "No matching import directive.");
        }
    ...
    add_build_configuration(params) {
        const store = (0, project_access_1.requireProjectStore)();
        ...
        if (projectBuildConfigs(store).some((c) => c.name === params.name)) {
            throw new protocol_1.BridgeError("BAD_PARAMS", `A build configuration named "${params.name}" already exists.`);
        }
        const cfg = (0, store_1.createObject)(store, { name: params.name }, project_1.BuildConfiguration);
    ...
    set_zoom(params) {
        ...
        if (!state || typeof state.transform === "undefined") {
            throw new protocol_1.BridgeError("NO_PROJECT", "No active page editor to zoom. Open a page first.");
        }
        const mode = params.mode;
        if (!mode) {
            throw new protocol_1.BridgeError("BAD_PARAMS", "mode is required (level | fit | center | reset).");
        }
        if (mode === "reset") {
        ...
        if (mode === "level") {
            if (typeof params.level !== "number") {
                throw new protocol_1.BridgeError("BAD_PARAMS", "level (number) is required for mode 'level'.");
            }
        ...
        if (mode === "fit") {
            ...
            if (!t.clientRect || !t.clientRect.width) {
                throw new protocol_1.BridgeError("UNSUPPORTED", "fit requires a mounted editor viewport (clientRect); use mode 'reset' or a numeric level.");
            }
            const pr = state.flow.pageRect;
            const margin = 1.1;
            const scale = Math.min(t.clientRect.width / (pr.width * margin), t.clientRect.height / (pr.height * margin));
            if (store.uiStateStore.globalFlowZoom) {
        ...
        throw new protocol_1.BridgeError("BAD_PARAMS", `Unknown zoom mode "${mode}" (expected level | fit | center | reset).`);
    },
    // --- Readme --------------------------------------------------------------
    ...
    set_readme(params) {
        ...
        if (!params.readmeFile) {
            throw new protocol_1.BridgeError("BAD_PARAMS", "readmeFile is required.");
        }
        const relPath = store.getFilePathRelativeToProjectPath(params.readmeFile);
        store.undoManager.setCombineCommands(true);
        try {
            if (!project.readme) {
                const feature = requireFeature("readme");
                const readmeObj = (0, store_1.createObject)(store, feature.create(), feature.typeClass, "readme");
