...
exports.assetsHandlers = void 0;
const store_1 = require("project-editor/store");
const commands_1 = require("project-editor/store/commands");
const bitmap_1 = require("project-editor/features/bitmap/bitmap");
...
function resolveColorFormat(store, colorFormat) {
    if (colorFormat === undefined || colorFormat === null) {
        return lvgl_constants_1.CF_TRUE_COLOR_ALPHA;
    }
    ...
    if (typeof colorFormat === "number") {
        ...
        if (!byId) {
            throw new protocol_1.BridgeError("BAD_PARAMS", `Unknown colorFormat id ${colorFormat}.`);
        }
        return byId.id;
    }
    ...
    if (!byLabel) {
        throw new protocol_1.BridgeError("BAD_PARAMS", `Unknown colorFormat "${colorFormat}".`);
    }
    return byLabel.id;
}
exports.assetsHandlers = {
    ...
    async add_bitmap(params) {
        ...
        if (name) {
            (0, project_access_1.assertNameFree)((0, project_access_1.projectBitmaps)(store), name, "bitmap");
        }
        else {
            name = (0, commands_1.getUniquePropertyValue)(store.project.bitmaps, "name", base);
        }
        // Build the persisted image string: embedded data URL (default) or rel path.
        let image;
        if (general.embedBitmaps) {
            if (params.dataBase64) {
                const b64 = params.dataBase64;
                ...
                const fs = require("fs");
                const bytes = fs.readFileSync(params.filePath, "base64");
                const fileType = params.fileType || fileTypeFromExt(params.filePath);
                image = `data:${fileType};base64,` + bytes;
            }
            else {
                throw new protocol_1.BridgeError("BAD_PARAMS", "Either dataBase64 or filePath is required.");
            }
        }
        else {
            if (!params.filePath) {
                throw new protocol_1.BridgeError("BAD_PARAMS", "filePath is required when embedBitmaps is disabled.");
            }
            image = store.getFilePathRelativeToProjectPath(params.filePath);
        }
        const bpp = resolveColorFormat(store, params.colorFormat);
        const seed = { name, image, bpp, alwaysBuild: false };
        const bitmap = (0, store_1.createObject)(store, seed, bitmap_1.Bitmap);
    ...
    delete_bitmap(params) {
        const store = (0, project_access_1.requireProjectStore)();
        const bitmap = (0, project_access_1.resolveByName)((0, project_access_1.projectBitmaps)(store), params.name, "bitmap");
    ...
    async add_font(params) {
        ...
        if (name.indexOf(".") !== -1) {
            throw new protocol_1.BridgeError("BAD_PARAMS", 'LVGL font name must not contain "." characters.');
        }
        (0, project_access_1.assertNameFree)((0, project_access_1.projectFonts)(store), name, "font");
        if (!params.filePath) {
        ...
        if (typeof size !== "number" || typeof bpp !== "number") {
            throw new protocol_1.BridgeError("BAD_PARAMS", "size (pixels) and bpp (1|2|4|8) are required numbers.");
        }
        const lvglRanges = params.ranges ?? "";
        const lvglSymbols = params.symbols ?? "";
        // Validate ranges up-front (getLvglEncodingsAndSymbols does getEncodings(...)!).
        if ((0, font_1.getEncodings)(lvglRanges) === undefined) {
            throw new protocol_1.BridgeError("BAD_PARAMS", `Invalid ranges string "${lvglRanges}".`);
        }
        const { encodings, symbols } = (0, font_1.getLvglEncodingsAndSymbols)(lvglRanges, lvglSymbols);
        const relativeFilePath = store.getFilePathRelativeToProjectPath(params.filePath);
        let fontProperties;
        try {
            fontProperties = await (0, font_extract_1.extractFont)({
                name,
        ...
        fontProperties.lvglRanges = lvglRanges;
        fontProperties.lvglSymbols = lvglSymbols;
        const font = (0, store_1.createObject)(store, fontProperties, font_1.Font);
    ...
    async edit_font(params) {
        const store = (0, project_access_1.requireLvglStore)();
        const font = params.name
            ? (0, project_access_1.resolveByName)((0, project_access_1.projectFonts)(store), params.name, "font")
            : (0, project_access_1.resolveObject)(store, params.objID);
        const newBpp = params.bpp ?? font.bpp;
        const newSize = params.size ?? font.source?.size;
        const newRanges = params.ranges ?? font.lvglRanges ?? "";
        const newSymbols = params.symbols ?? font.lvglSymbols ?? "";
        if (typeof newSize !== "number") {
            throw new protocol_1.BridgeError("BAD_PARAMS", "size is required (font has no existing source size).");
        }
        ...
        const relPath = params.filePath
            ? store.getFilePathRelativeToProjectPath(params.filePath)
            : font.source?.filePath;
        if (!relPath) {
            throw new protocol_1.BridgeError("BAD_PARAMS", "font has no source file; filePath is required.");
        }
    ...
    delete_font(params) {
        const store = (0, project_access_1.requireProjectStore)();
        const font = (0, project_access_1.resolveByName)((0, project_access_1.projectFonts)(store), params.name, "font");
